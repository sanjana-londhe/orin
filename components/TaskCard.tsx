...
function TaskCardInner({
  ...
  // ── Inline edit (same creation form aesthetic) ────────────────────
  if (editing) {
    return (
  ...
  return (
    <>
      ...
      >
        {/* ── Header: emotion pill + deferred badge + actions ── */}
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 9 }}>
          ...
          {!isLocallyCompleted && (
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 4, opacity: hovered ? 1 : 0, transition: "opacity 0.15s" }}>
        ...
        {(due || (!isLocallyCompleted && onDefer)) && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: totalSubs > 0 ? 10 : 0 }}>
            ...
            {!isLocallyCompleted && onDefer && (
              <button onClick={() => setDeferOpen(true)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 11.5, fontWeight: 600, color: "#059669", padding: 0, fontFamily: "inherit" }}
        ...
        {totalSubs > 0 && (
          <div style={{ marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                {task.subtasks.map(sub => (
                  <div key={sub.id} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    {!isLocallyCompleted && (
                      <button onClick={() => onDeleteSubtask?.(sub.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "#e8ece8", padding: 0, display: "flex", opacity: 0, transition: "opacity 0.15s" }}
                        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.opacity = "1"; (e.currentTarget as HTMLElement).style.color = "#c23934"; }}
            ...
            {totalSubs > 1 && (
              <button onClick={() => setSubsExpanded(o => !o)} style={{ marginTop: 6, background: "none", border: "none", cursor: "pointer", fontSize: 11, color: "#b9d3c4", fontFamily: "inherit", padding: 0 }}
              ...
              >{subsExpanded ? "Hide subtasks" : `Show ${totalSubs} subtasks`}</button>
            )}
          </div>
        )}
        ...
        {!isLocallyCompleted && (
          showSubInput ? (
            ...
            <button onClick={() => setShowSubInput(true)} style={{ display: "inline-flex", alignItems: "center", gap: 5, marginBottom: 10, background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "#b9d3c4", fontFamily: "inherit", padding: 0 }}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, paddingTop: 10, borderTop: "1px solid #f0f3f0" }}>
          ...
          <span onClick={() => onMarkDone?.(task.id)} style={{
            fontSize: 12, fontWeight: 500,
            color: isLocallyCompleted ? "#059669" : "#b9d3c4",
            cursor: "pointer",
          }}
            onMouseEnter={e => { if (!isLocallyCompleted) (e.currentTarget as HTMLElement).style.color = "#3d5a4a"; }}
            onMouseLeave={e => { if (!isLocallyCompleted) (e.currentTarget as HTMLElement).style.color = "#b9d3c4"; }}
