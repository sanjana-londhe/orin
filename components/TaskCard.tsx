...
function formatDue(dueAt: Date | string | null) {
  ...
  return { label: (isToday || isTomorrow) ? `${date} · ${time}` : date, overdue, isoDate: d.toISOString().slice(0,10), isoTime: d.toISOString().slice(11,16) };
}
  ...
  return (
    <>
      ...
      >
        ...
        {/* card-title */}
        <div style={{ fontSize:15, fontWeight:700, color:isLocallyCompleted?"#b9d3c4":"#082d1d", marginBottom:8, lineHeight:1.35, textDecoration:isLocallyCompleted?"line-through":"none" }}>
          {task.title}
        </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:5, marginBottom:12 }}>
            {task.subtasks.map(sub => (
              <div key={sub.id} style={{ display:"flex", alignItems:"center", gap:8 }}>
                ...
                {/* subtask-label */}
                <span style={{ fontSize:12.5, fontWeight:500, color:sub.isCompleted?"#b9d3c4":"#3d5a4a", textDecoration:sub.isCompleted?"line-through":"none", flex:1 }}>
                  {sub.title}
                </span>
                {!isLocallyCompleted && (
                  <span onClick={()=>onDeleteSubtask?.(sub.id)} style={{ color:"#e8ece8", cursor:"pointer", fontSize:12, display:"flex" }}
        ...
        {/* add subtask */}
        {!isLocallyCompleted && (showSubInput ? (
          ...
          <div onClick={()=>setShowSubInput(true)} style={{ display:"inline-flex", alignItems:"center", gap:5, marginBottom:12, cursor:"pointer", fontSize:12, color:"#b9d3c4" }}
          ...
          ><Plus size={11} strokeWidth={2.5} /> Add subtask</div>
        ))}
